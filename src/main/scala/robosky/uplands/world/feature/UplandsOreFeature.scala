...
object UplandsOreFeature extends Feature[UplandsOreFeatureConfig]((t: datafixers.Dynamic[_]) =>
  UplandsOreFeatureConfig.deserialize(t)) {
  ...
  def generate(world: IWorld, generator: ChunkGenerator[_ <: ChunkGeneratorConfig], rand: Random, pos: BlockPos,
    config: UplandsOreFeatureConfig): Boolean = {
